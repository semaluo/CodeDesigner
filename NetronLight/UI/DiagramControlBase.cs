/*
FIX2006020601：使DiagramControlBase支持滚动条
修复以后任何继承DiagramControlBase的控件只要设置AutoScroll = true;
以及设置AutoScrollMinSize后就可以使控件自动支持滚动
做了一下代码更改
1.在OnScroll事件里添加Invalidate()调用，使滚动事件发生时控件重画
2.在OnPaint事件里调用Graphics.TranslateTransform做坐标变换
...
namespace Netron.NetronLight
{
    ...
    public abstract class DiagramControlBase : ScrollableControl, ISupportInitialize, IDiagramControl
    {
        ...
        In des Herzens heilig stille R鋟me
        Mu遲 du fliehen aus des Lebens Drang!
        Freiheit ist nur in dem Reich der Tr鋟me,
        Und das Sch鰊e bl黨t nur im Gesang.
        (F.Shiller)
